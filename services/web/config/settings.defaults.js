...
module.exports = {
  ...
  redis: {
    web: (() => {
      // HA Redis (§D16): when REDIS_SENTINELS is set (JSON array of
      // {host, port}), ioredis connects through the sentinel fleet for
      // automatic master failover instead of a fixed host.
      const webBase = {
      ...
      if (process.env.REDIS_SENTINELS) {
        try {
          ...
          console.error("invalid REDIS_SENTINELS, falling back to host", e);
        }
      }
      return {
        ...webBase,
