...
// Hot-path source of truth for admin suspensions of logged-in sessions.
// Created lazily: module-level client creation would make this module
// un-importable in tests that stub a minimal Settings object.
let _suspendedUsersClient;
function suspendedUsersClient() {
  if (_suspendedUsersClient == null) {
    _suspendedUsersClient = RedisWrapper.client("suspended_users");
  }
  return _suspendedUsersClient;
}
  ...
  requireLogin() {
    const doRequest = async function (req, res, next) {
      ...
      if (!SessionManager.isUserLoggedIn(req.session)) {
        ...
        return AuthenticationController._redirectToLoginOrRegisterPage(
          req,
          res,
        );
      } else {
        req.user = SessionManager.getSessionUser(req.session);
        // Enforce admin suspensions on live sessions: the Redis set is the
        // hot-path source of truth (fail-open with a logged error so a
        // Redis hiccup cannot lock out the whole instance).
        try {
          const suspended = await suspendedUsersClient().sismember(
            "suspended_users",
            String(req.user._id),
          );
          if (suspended === 1) {
            req.session.destroy(() => {});
            if (acceptsJson(req)) {
              return res.status(403).json({ status: "suspended" });
            }
            return res.redirect("/login?suspended=1");
          }
        } catch (err) {
          logger.warn({ err }, "failed to check suspension state");
        }
