...
async function generateInitDoc(projectId, userId) {
  ...
  // Style and terminology notes from heuristic analysis of the .tex
  // sources (PLANS 12 "style.md" / "terminology.md").
  const texDocs = [];
  for (const f of files) {
    if (f.type === "doc" && f.path.endsWith(".tex")) {
      const lines = await readDocLines(projectId, f.path);
      if (lines != null) texDocs.push({ path: f.path, text: lines.join("\n") });
    }
  }
  const allTex = texDocs.map((d) => d.text).join("\n");
  if (allTex) {
    const packages = [
      ...new Set(
        [...allTex.matchAll(/\\usepackage(?:\[[^\]]*\])?\{(\w+)\}/g)].map(
          (m) => m[1],
        ),
      ),
    ].slice(0, 15);
    const sections = [
      ...new Set(
        [...allTex.matchAll(/\\(sub)?section\{([^}]+)\}/g)].map((m) => m[2]),
      ),
    ].slice(0, 12);
    const acronyms = [
      ...new Set(
        [...allTex.matchAll(/\b([A-Z]{2,6})\b/g)]
          .map((m) => m[1])
          .filter(
            (a) =>
              !["PDF", "PNG", "JPEG", "TODO", "XML", "HTML", "CSS"].includes(a),
          ),
      ),
    ].slice(0, 15);

    const styleLines = [
      "# Writing style notes (generated by /init)",
      "",
      `Document class: ${documentClass || "not detected"}`,
      `Language: ${language}`,
      packages.length > 0
        ? `Packages in use: ${packages.join(", ")}`
        : "Packages: none detected",
      sections.length > 0 ? `Section flow: ${sections.join(" -> ")}` : "",
      "",
      "## Conventions",
      "",
      "- Match the existing voice and tense of the document",
      "- Keep section structure stable unless asked otherwise",
      "",
      "Add author style preferences below.",
    ].filter((l) => l !== null && l !== "");
    await EditorController.promises.upsertDocWithPath(
      projectId,
      "/style.md",
    ...
    written.push({ path: "style.md", lines: styleLines });

    if (acronyms.length > 0) {
      const termLines = [
        "# Terminology (generated by /init)",
        "",
        "Acronyms detected in the project (preserve as-is, define on first use):",
        "",
        ...acronyms.map((a) => `- ${a}`),
        "",
      ...
      written.push({ path: "terminology.md", lines: termLines });
    }
  }
