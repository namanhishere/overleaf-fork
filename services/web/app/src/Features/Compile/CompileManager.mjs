...
async function compile(projectId, userId, options = {}) {
  ...
  try {
    ...
    if (err instanceof CompileJobManager.CompileLimitReachedError) {
      return { status: "compile-limit-reached", outputFiles: [], jobId };
    }
    throw err;
  }

  const job = await CompileJobManager.startJob({
    jobId,
    projectId,
    userId,
  ...
  try {
    ...
    await CompileJobManager.finishJob(jobId, {
      status: result.status === "success" ? "success" : "failed",
      error:
        result.status !== "success" ? `compile status: ${result.status}` : null,
      stats: {
        workerId: result.clsiServerId || null,
      },
    });
    return { ...result, jobId };
  } catch (error) {
    ...
    await CompileJobManager.releaseAllSlots(userId, projectId, jobId).catch(
      () => {},
    );
  }
}
...
async function stopCompile(projectId, userId) {
  ...
  await ClsiManager.promises.stopCompile(projectId, userId, limits);
  // Mark the in-flight job record(s) cancelled. The killed compile also
  // surfaces as an error in its dispatching request; finishJob() ignores
  // finalizations of rows that are no longer queued/running.
  await CompileJobManager.cancelActiveJobs(projectId, "cancelled by user");
}
