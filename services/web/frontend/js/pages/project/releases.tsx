...
function projectIdFromPath() {
  const match = window.location.pathname.match(/\/project\/([a-f0-9]{24})\/releases/)
    ...
    try {
      const [rel, jobData] = await Promise.all([
        getJSON<{ releases: Release[] }>(
          `/project/${projectId}/api/releases`
        ),
        getJSON<{ jobs: CompileJob[] }>(
          `/project/${projectId}/jobs?limit=10`
        ),
      ])
