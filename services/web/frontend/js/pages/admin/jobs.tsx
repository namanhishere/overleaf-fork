...
function JobsDashboard() {
  ...
  async function refresh() {
    const data = await getJSON<{ jobs: CompileJob[] }>('/admin/api/jobs?limit=50')
    setJobs(data.jobs)
    return data.jobs
  }
  ...
  useEffect(() => {
    ...
    async function tick() {
      ...
      timer = setInterval(tick, current.some(j => j.status === 'running') ? 5000 : 15000)
    }
    tick()
    ...
    <div>
      ...
      {logText ? (
        <div className="alert alert-default">
          ...
          <button className="btn btn-xs btn-default" onClick={() => setLogText(null)}>
            Close log
          </button>
        </div>
      ) : null}
        ...
        <tbody>
          {jobs.map(job => (
            <tr key={job.jobId}>
              ...
              <td>{job.workerId || '—'}</td>
              <td>{job.status}</td>
              <td>{job.runtimeMs != null ? `${(job.runtimeMs / 1000).toFixed(1)}s` : '—'}</td>
              <td>{job.peakCpuPercent != null ? `${job.peakCpuPercent}%` : '—'}</td>
