...
import { getJSON, putJSON, postJSON, deleteJSON } from '@/infrastructure/fetch-json'
...
function userIdFromPath() {
  const match = window.location.pathname.match(/\/admin\/users\/([a-f0-9]{24})/)
...
function UserDetail() {
  ...
  async function deleteUser() {
    ...
    await deleteJSON(`/admin/api/users/${userId}?confirm=${encodeURIComponent(user.email)}`)
      ...
      <table className="table">
        <tbody>
          ...
          <tr>
            <th>Last login</th>
            <td>
              {user.lastLoggedIn || '—'} {user.lastLoginIp ? `(${user.lastLoginIp})` : ''}
            </td>
          </tr>
          <tr>
            <th>Signed up</th>
