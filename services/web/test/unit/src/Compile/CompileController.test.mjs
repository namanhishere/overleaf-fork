...
describe("CompileController", function () {
  beforeEach(async function (ctx) {
    ...
    ctx.CompileJobManager = {
      startJob: sinon.stub().resolves({ jobId: "job-1" }),
      acquireAllSlots: sinon.stub().resolves(),
      releaseAllSlots: sinon.stub().resolves(),
      markStarted: sinon.stub().resolves(),
      dispatch: sinon.stub().callsFake((jobId, executor) => executor()),
      finishJob: sinon.stub().resolves(),
      cancelActiveJobs: sinon.stub().resolves([]),
      CompileLimitReachedError: class extends Error {},
    };
    ...
    vi.doMock("../../../../app/src/Features/Compile/CompileJobManager", () => ({
