...
import LockManager from "./app/js/LockManager.js";
...
app.get("/health_details", function (req, res) {
  const concurrency = LockManager.getConcurrency();
  res.json({
    ok: !ProjectPersistenceManager.isAnyDiskCriticalLow(),
    concurrency,
    disk: { freePct: ProjectPersistenceManager.getDiskFreePercent() },
    uptimeS: Math.floor(process.uptime()),
    versions: { clsi: Settings.clsi?.version || "unknown" },
  });
});
